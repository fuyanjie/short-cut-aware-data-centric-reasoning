...
We employ a GPT-style transformer with 6 layers, hidden dimension $d_{\text{model}} = 512$, 8 attention heads, and feed-forward dimension $d_{\text{ff}} = 2048$, totaling approximately 19.0M trainable parameters. The model uses learned positional embeddings and a vocabulary of 35 tokens. We set dropout to 0 to eliminate train--eval mode discrepancies that can confound gradient-based analysis.
...
The entire experimental pipeline---including training all methods across three datasets, computing per-sample gradients for \textsc{ShortcutScore} estimation, and running all evaluations---completed in under 2 minutes on a single NVIDIA H100 NVL GPU. Our method's two-phase design (standard warmup followed by scored retraining) adds only modest overhead relative to Standard FT, attributable primarily to the one-time per-sample gradient computation in Phase~2. This demonstrates that gradient-aware shortcut correction is practical even for rapid experimental iteration.
